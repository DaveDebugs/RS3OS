    ...
    override fun channelRead0(ctx: ChannelHandlerContext, msg: HandshakeRequest) {
        ...
        when (msg.type) {
            HandshakeType.JS_5 -> {
                ...
                // Diagnostic only (opt-in): a pass-through observer at the pipeline
                // tail, where outbound writes start, so it sees each
                // Js5Packet.RequestFileResponse - i.e. which requested files were
                // actually found and handed to the encoder - before Js5Encoder turns
                // it into bytes. No-op when off.
