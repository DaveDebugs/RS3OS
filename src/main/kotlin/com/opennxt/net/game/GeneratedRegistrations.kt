...
/**
 * Registrations for every generated packet class (declared in data/prot/949 but
 * carrying no hand-written class). [registerAll] runs from PacketRegistry.reload();
 * [installClientHandlers] gives each generated client->server packet the decoded
 * logging handler, so a frame that used to arrive as an observed blob now logs its
 * fields by name. Names are looked up through the same table as everything else, so
 * a name this build has no opcode for is reported as unmapped, never invented.
 */
object GeneratedRegistrations {
    ...
    /** Every generated client->server packet class, for handler tables and checks. */
    ...
    /** Every generated server->client packet class. */
