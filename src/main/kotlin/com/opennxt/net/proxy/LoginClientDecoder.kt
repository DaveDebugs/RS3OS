...
class LoginClientDecoder : ByteToMessageDecoder() {
 ...
 // HARDENED: a raw hexdump of the login response
                    // frame went to stdout unconditionally. It is not the password
                    // (that lives in the client->server RSA block, never here), but a
                    // login frame is not something to print by default. Gated off.
