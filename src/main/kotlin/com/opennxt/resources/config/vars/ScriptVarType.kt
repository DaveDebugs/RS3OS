...
/**
 * This server's transcription of the client's config type table.
 */
enum class ScriptVarType(
    ...
    /**
     *. Left UNNAMED on purpose - see [com.opennxt.resources.sqlite.CacheVarTypes].
     */
    ...
    companion object {
        ...
        init {
 //: entries whose char is NUL are the ones this server
            // does not know the char OF, not ones whose char is NUL. Registering
            // them all sent every such entry to slot 63 ('?' - what
            // TextUtils.charToCp1252 returns for anything it cannot encode), so
            // getByChar('?') returned whichever of them was declared last, which
            // is an answer with no evidence behind it. They are skipped instead,
            // and getByChar('?') now throws.
            //
            // Unreachable on the served cache either way, and that is measured
            // rather than assumed: NO param config in index 2 archive 11 uses
            // the char opcode at all (0 of 9,472; 6,016 use opcode 101), and the
            // only two enum configs that still do - 953 and 954 of 16,686 -
            // use only the chars 'i' and 's'. Those four counts are a ONE-OFF measurement,
 //, reproducible with `python tools/vt209.py --opcodes`;
            // is the lookup behaviour this comment is justifying, not the
            // counts.
            values.forEach { type ->
        ...
        /** The `c` column's "this server does not know the char" marker. */
