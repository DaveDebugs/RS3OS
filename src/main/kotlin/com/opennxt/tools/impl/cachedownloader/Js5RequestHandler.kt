                ...
                synchronized(lock) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (request.isCompleted()) {
                            ...
                            it.remove()
                            // Without this, a request that is both completed and
                            // flagged crashed falls into the branch below and
                            // calls remove() a second time on the same position -
                            // IllegalStateException, which the enclosing catch
                            // turns into exitProcess(1).
                            continue
                        }
                    ...
                    while (it2.hasNext()) {
                        ...
                        if (request.crashed) {
                            // This said `it.remove()` - the iterator over the JS5
                            // `processing` set, which is exhausted by this point.
                            // Two failures in one line: a crashed HTTP request was
                            // never removed from processingHttp, so pendingHttpCount
                            // could never reach zero and the tool could never satisfy
                            // its shutdown condition; and remove() on that spent
                            // iterator throws IllegalStateException, which the
                            // enclosing catch turns into exitProcess(1).
                            //
                            // The second half never actually fired, because
                            // Js5HttpRequest had no error handling and so never set
                            // `crashed` on anything. Giving that class real error
                            // handling is what makes this line reachable.
                            it2.remove()
                            crashed.add(request)
                        }
                    }
